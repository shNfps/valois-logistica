...
export async function createPedido(cliente,motorista,cidade,orcamentoUrl,criadoPor,numeroRefCustom,clienteId,formaPagamento,prazoDias,obsComercial){
  ...
  const obs_comercial=(obsComercial||'').trim()||null
  const{data,error}=await supabase.from('pedidos').insert({cliente,motorista,cidade,orcamento_url:orcamentoUrl,status:'PENDENTE',criado_por:criadoPor,numero_ref,cliente_id:clienteId||null,forma_pagamento:formaPagamento||'a_vista',prazo_pagamento_dias:prazoDias||0,obs_comercial}).select().single()
...
export async function addRotaPedidos(rotaId,pedidoIds){const rows=pedidoIds.map(pid=>({rota_id:rotaId,pedido_id:pid}));const{error}=await supabase.from('rota_pedidos').insert(rows);if(error)console.error(error);return{error:error||null}}
// Atualiza status de vários pedidos em uma única ida ao Supabase (atômico no servidor).
export async function updatePedidosStatus(ids,updates){if(!ids?.length)return{error:null};const{error}=await supabase.from('pedidos').update(updates).in('id',ids);if(error)console.error(error);return{error:error||null}}
